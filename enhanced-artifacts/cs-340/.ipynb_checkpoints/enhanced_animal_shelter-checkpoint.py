...
class AnimalShelter:
    ...
    def get_rescue_stats(self, rescue_type=None):
        ...
        if rescue_type == "Water":
            pipeline[0]["$match"]["breed"] = {"$in": ["Labrador Retriever Mix", "Chesapeake Bay Retriever", "Newfoundland"]}
            pipeline[0]["$match"]["sex_upon_outcome"] = "Intact Female"
            pipeline[0]["$match"]["age_upon_outcome_in_weeks"] = {"$gte": 26, "$lte": 156}
        elif rescue_type == "Mountain":
            pipeline[0]["$match"]["breed"] = {"$in": ["German Shepherd", "Alaskan Malamute", "Old English Sheepdog", "Siberian Husky", "Rottweiler"]}
            pipeline[0]["$match"]["sex_upon_outcome"] = "Intact Male"
            pipeline[0]["$match"]["age_upon_outcome_in_weeks"] = {"$gte": 26, "$lte": 156}
        elif rescue_type == "Disaster":
            pipeline[0]["$match"]["breed"] = {"$in": ["Doberman Pinscher", "German Shepherd", "Golden Retriever", "Bloodhound", "Rottweiler"]}
            pipeline[0]["$match"]["sex_upon_outcome"] = "Intact Male"
